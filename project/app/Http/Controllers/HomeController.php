...
// return response()->file($file, $headers);
    ...
    {
      $totalPoint = CusPoint::where('user_id', auth()->user()->id)->sum('name');
      $listPoint = PointList::all();
      ...
      $userR = Review::where('user_id', auth()->user()->id)->first();
        return view('front.user.myAccount', compact('userR', 'totalPoint','listPoint'));
    }
    ...
    {
         $pli = \App\Models\PointList::all();
        $orderSum = Order::where('user_id', auth()->user()->id)->sum('pay_amount');

        foreach ($pli as $key => $val) {
          if ($orderSum > $val->total) {
            $cusPointE = CusPoint::where('name', $val->name)->where('user_id', auth()->user()->id)->exists();
            if (!$cusPointE) {
              CusPoint::create([
                'name' => $val->name,
                'total' => $val->total,
